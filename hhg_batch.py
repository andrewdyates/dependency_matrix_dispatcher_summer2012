...
class HHGBatch(Batch):
  ...
  def compute(self, i, x, y):
    ...
    lg = np.log(2)
    self.Matrices["SUM_CHI"] = float(HHG.rx('sum_chisquared')[0][0]) / v
    self.Matrices["SUM_LR"]  = float(HHG.rx('sum_lr')[0][0]) / v / lg
    self.Matrices["MAX_CHI"] = float(HHG.rx('max_chisquared')[0][0]) / (n-2)
    self.Matrices["MAX_LR"]  = float(HHG.rx('max_lr')[0][0]) / (n-2) / lg
